...
ALTER TABLE ${schema}.registry_entries DROP CONSTRAINT IF EXISTS registry_entries_status_check;
ALTER TABLE ${schema}.registry_entries ADD CONSTRAINT registry_entries_status_check
    CHECK (status IN ('draft', 'dev', 'testing', 'pending', 'live', 'deprecated'));

ALTER TABLE ${schema}.registry_audit DROP CONSTRAINT IF EXISTS registry_audit_action_check;
ALTER TABLE ${schema}.registry_audit ADD CONSTRAINT registry_audit_action_check
