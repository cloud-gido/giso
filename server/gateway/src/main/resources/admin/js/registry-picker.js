...
export function renderPoolPicker(name, selected, registry, fieldLabel) {
  ...
  return `<div class="pool-picker" data-field="${esc(name)}" data-pool="${esc(cfg.poolKind)}">
    ...
    <div class="pool-search-row">
      ...
      <button type="button" class="ghost pool-goto" data-goto="${esc(cfg.poolKind)}">去${esc(cfg.poolLabel)}配置</button>
    </div>
...
function bindMultiPicker(picker, registry, cfg) {
  ...
  picker.querySelector('.pool-goto')?.addEventListener('click', () => {
    picker.dispatchEvent(new CustomEvent('pool-goto', {
      bubbles: true,
      detail: { poolKind: cfg.poolKind },
    }));
  });
...
export function saveEditorResume(data) {
  sessionStorage.setItem(RESUME_KEY, JSON.stringify({ ...data, ts: Date.now() }));
}

export function peekEditorResume() {
  try {
...
export function clearEditorResume() {
