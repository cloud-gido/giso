...
**RBAC 两层模型（PostgreSQL 模式）**

| 层 | 存储 | 角色 |
|----|------|------|
| 平台 | `admin_users.role` | `system_admin` · `user` |
| 空间 | `space_members.role` | `space_admin` · `editor` · `viewer` |

- 种子 `viewer`/`editor`（`auth.users` 或 `GISO_VIEWER_*`）启动时自动加入全部 active 空间。
- 管理台创建平台用户时须指定空间角色（`POST /admin/api/users` 支持 `space_key` / `space_role` / `all_spaces`），否则无空间授权无法登录。
- 非平台管理员登录前校验至少属于一个空间；`/me` 在当前空间无权限时回退到首个可访问空间。
- 代码入口：`RbacRoles` · `RbacProvisioning` · `AdminPermissions`。
